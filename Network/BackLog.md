...
windows에서는 조절이 실질적으로 불가능하며(영향을 주는 레지스트리키는 있으나 무조건 반영되지는 않음.),<br/>
windows버전에 따라 ```200~2000```수준으로 제한되어있을 수 있다. <br/>

## net.ipv4.tcp_max_syn_backlog
위에서 언급한 ```somaxconn```의 경우는 연결할 준비를 마친 요청들의 대기 큐라면, <br/>
```tcp_max_syn_backlog```는 절반만 연결한 요청들의 대기 큐의 크기이다. <br/>
```3-way-handshake```의 클라이언트 요청이 서버에 도착하여 처리되고, 서버에서 다시 클라이언트의 ```ACK```를 기다리고 있는 요청들의 큐. <br/>
...
### Windows에서는 커널 큐 튜닝이 안되는거면 폭발적인 연결요청을 감당못할까?
```AcceptEx```로 예약을 많이 걸어두는 방식으로 어느정도 대체가 가능하다. <Br/>
유저모드 <-> 커널모드 전환이 적기 때문에 빠르게 처리되어 해결되는 부분도 있고, <br/>
OS에서 어느정도 조절을 해주기도 한다. 명시적 조절이 불가능한 것 뿐. <Br/>
